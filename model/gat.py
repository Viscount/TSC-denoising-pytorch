...
class GAT(Module):
    def __init__(self, in_feats, hidden_size, dropout):
        ...
        self.conv1 = GATConv(in_feats, hidden_size, heads=8)
        self.conv2 = GATConv(hidden_size, in_feats, heads=8)
        self.dropout = dropout

        self.fc1 = nn.Linear(in_feats, 256, bias=True)
        self.fc2 = nn.Linear(256, 2, bias=True)

    def forward(self, graph, sentence):
        ...
        sent_emd = F.dropout(sent_emd, p=self.dropout, training=self.training)
...
def train(season_id, dm_train_set, dm_test_set, features, edges):
    ...
    model = GAT(EMBEDDING_DIM, 256, dropout=0.5)
    print(model)
    model.to(device)
